...
Route::group(['middleware' => 'auth:sanctum'], static function () {
    ...
    Route::get('orders', [OrdersController::class, 'index'])
        ->name('orders.index');
    // 单据 - 商务单据
    Route::get('orders/commerce-index', [OrdersController::class, 'commerceIndex'])
        ->name('orders.commerce-index');
    // 单据 - 财务单据
    ...
    // 单据账单 - 列表
    Route::get('order-bills', [OrderBillsController::class, 'index'])
        ->name('order-bills.index');
    // 单据账单 - 新增
    ...
    // 单据账单 - 编辑
    Route::put('order-bills/{orderBill}', [OrderBillsController::class, 'update'])
        ->name('order-bills.update')->where('orderBill', '[0-9]+');
    // 单据账单 - 删除
    ...
    Route::put('invoices/{invoice}', [InvoicesController::class, 'update'])
        ->name('invoices.update')->where('invoice', '[0-9]+');
    // 发票 - 删除
